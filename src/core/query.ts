...
export class Query<TResult, TError> {
  ...
  scheduleStaleTimeout(): void {
    ...
    this.staleTimeout = setTimeout(() => {
      if (this.queryCache.getQuery(this.queryKey)) {
        this.invalidate()
      }
  ...
  subscribe(onStateUpdate = noop): QueryInstance<TResult, TError> {
      ...
      if (fetchMore) {
        queryFn = async (...args: any[]) => {
          try {
            ...
            const newArgs = [...args, fetchMoreVariable]
...
function getDefaultState<TResult, TError>(
  ...
  return {
    ...
    error: null,
    isFetching: initialStatus === 'loading',
    failureCount: 0,
    isStale,
    markedForGarbageCollection: false,
    data: initialData,
    updatedAt: hasInitialData ? Date.now() : 0,
    ...getStatusBools(initialStatus),
  }
}
...
function switchActions<TResult, TError>(
  state: QueryState<TResult, TError>,
  action: Action<TResult, TError>
): QueryState<TResult, TError> {
  switch (action.type) {
