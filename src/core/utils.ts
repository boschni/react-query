...
export function getQueryArgs<TResult, TError, TOptions = undefined>(
  ...
  if (isObject(args[0])) {
    ...
    args = [queryKey, queryFn, config, args[1]]
  } else if (isObject(args[1])) {
    const [queryKey, config, options] = args
    args = [queryKey, undefined, config, options]
  }

  const [queryKey, argQueryFn, argConfig, options] = args

  let config = argConfig || {}

  const queryFn = argQueryFn || config.queryFn
...
export function getStatusBools(status: QueryStatus) {
  return {
